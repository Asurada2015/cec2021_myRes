...
public class MOEAD_AE extends Algorithm {
     ...
     */
    public MOEAD_AE(ProblemSet problemSet) {
        super(problemSet);
...
//                    }
            ...
            for (int i = 0; i < tasknum; i++){
                for (int j = 0; j < tasknum; j++){
                    if (j == i) continue;;
                    da[num].net.fit(new NDArray(population_.getIndividual(j, populationSize_)), new NDArray(population_.getIndividual(i, populationSize_)));
                    num++;
                }
            }
...
//                    if (maxEvaluations % 1000 == 0){
                    if (tranRand <= 0.1){
                        ...
                        double[][] trans = da[daNum].net.output(new NDArray(child.getIndiVirables())).toDoubleMatrix();
                        child.setDecisionVariables(trans);
...
//            evaluations_++;
    ...
    private void initPopulation() throws ClassNotFoundException, JMException {
        ...
        //        init autoencoder
        int taskNum = problemSet_.size(), num = 0;
        da = new DenoisingAutoencoder[taskNum * (taskNum - 1)];
        for (int i = 0; i < taskNum; i++){
            for (int j = 0; j < taskNum; j++){
                if (j == i) continue;
                da[num] = new DenoisingAutoencoder(population_.getIndividual(j, populationSize_), population_.getIndividual(i, populationSize_));
                da[num].train();
                num++;
            }
        }
